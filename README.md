...
construction, and the only control for it is the gold set of 60 hand-labeled pairs.
...
(30 probes × 2 agents). DESIGN.md §4.2's reliability panel — Cohen's κ against
gold labels, per-class precision and recall, the false-alarm rate — is measured
in [`docs/results.md`](docs/results.md): pooled multi-class κ = 0.612,
over-promise precision 0.923 (12/13) and recall 0.571 (12/21), false-alarm rate
2.7% (1/37). The earlier κ of 0.847 was computed against LLM-generated
"cross-check" labels and was circular — the labeling LLM and the judge shared a
model family — so it is not comparable to the human-label number. The gold
labels also exposed a judge blind spot: the human labels found more over-promises
than the judge's live run (aut-naive 18/30 vs 11/30, aut-strong 5/30 vs 2/30).
...
tests/         unit / integration / gold labels (60 hand-labeled pairs)
