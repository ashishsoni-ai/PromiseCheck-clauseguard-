...
DESIGN.md §6.3 scripts a 55-second gate demo — green CI, edit "within 30 days" to
"within 7 days", push, watch CI go red. **That is not shootable**: the gate is not
built, `clauseguard check` deliberately refuses to run, and demoing it would mean
mocking the one thing a reviewer most wants to be real. It appears in this video
as a roadmap slide with the words "not built" on it, in the last forty seconds.
...
right at the top of the run. With `.env` supplying it you get one clean line —
`judge key  : .env` — and nothing about secrets.

Run from the repository root. `--probes`, `--rules`, `--manifest` and `--db` all
default to relative paths (`probes/probes.lock.json`, `runs.db`, …), so the
...
### 0:00 – 0:35 · The problem, in the agent's own words

*Screen:* the two turns and the reply as plain text, large, verbatim. Nothing
else. The words are quoted exactly as stored — this project's whole claim is that
...
> dispatch notification has just arrived — still fine to cancel, though?
>
...
### 0:35 – 1:10 · What it is, and the one thing that makes it not circular
...
> something the policy denies. The probes here are hand-written — the generator is
...
> the whole thing up.
>
> **The answer key is computed in Python** — a reviewed rule, evaluated over the
...
### 1:10 – 1:50 · The answer key, on screen

*Screen:* split view — `rules.lock.json` left, `probes.lock.json` right, both
...
> Here is the rule. One condition — `order_dispatched == yes` — a polarity, the
...
>
> And here is the probe. The customer's words, and a facts vector — this order was
...
realises — `item_category: footwear` on a probe about a desk lamp — a real defect
...
— then a progress line that overwrites itself, `agent: 7/30 probes` climbing to
...
The identity block — agent name, commit SHA, git tag, frozen-at timestamp —
...
```
    8× SPEED — the real run takes ~8 minutes.
...
> pre-filter settles the easy denials with no network call at all — ten of thirty
> on this run — and the rest go to the LLM judge, which has to cite a clause and
> quote from it.

### 2:50 – 3:30 · The number

*Screen:* back to real time, no cuts. The identity block lands first — say the
...
==============================================================================
  OVER-PROMISES: 11 / 30
  UNDER-SERVE: 0  ·  EVASIVE: 5  ·  JUDGE ABSTAINED: 0
==============================================================================
...
*The last sentence prints as one long line, not wrapped — don't be surprised when
...
Between the matrix and the failure table the run prints a history strip — *last N
run(s) — over-promises per run* — with a bar per run and a NEW/FIXED probe diff
...
> next to it deliberately — an agent that refuses everything would score zero
...
> so those two rows have no verdict, and they were both probes the policy denies —
...
### 3:30 – 4:20 · The evidence, and the one a human would have missed

*Screen:* scroll the failure table — `OVER-PROMISES (11), worst-evidenced last:`
— to `P-acme-018-multi_turn_drift-003`, the first entry. Each entry is stacked,
...
Get both marked spans in frame at once — that means the bottom half of the entry,
...
> with the contradicting sentence marked, and `L2: verified` — that quote was
...
*Note:* the marking is `>>` and `<<`, not colour — deliberately, so the output
...
> never reported it in time. Watch what the agent does. It reasons correctly —
...
> caught here because the check is against the clause, not against the tone — and
...
The judge cited the **correct** clause, `acme-refunds:015` — but that clause is a
...
written down in `docs/results.md` — one sentence on camera costs four seconds and
...
of four probes — the winning strategy is the one where the customer sounds like
they've read the policy.

### 4:20 – 5:00 · What is not built
...
> correct — only a record of what it did, mechanically verified wherever
> verification was possible.
>
> **The CI gate isn't wired.** `clauseguard check` exists and refuses to run,
> printing why, because a gate that exits zero without checking anything is worse
> than no gate.
>
...
already. Say so on camera and keep rolling — a harness that records provider
...
If you overrun, cut from *the answer key on screen* — it is the most
