...
landing on the over-promise cell and to the entire gold-label set" — and a `grants` → `denies`
flip *leaves* that cell, so it is never re-voted. **L3 as specified protects the
precision of the over-promise count, not its recall.** The gold-label set is the only control
...
correctness come apart here, and only the gold-label set measures the second one.
...
**What would reduce it,** cheapest first: a perturbation panel in the gold-label set, so the
