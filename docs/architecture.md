...
is C3.* Beside it, drawn dashed and greyed, a second identical container labelled
`aut-strong` — **not built**. An arrow from `probes.lock.json` (column 1) crosses
...
| **L3** | `consistency.py` — k=3 at temp 0.3, majority | over-promise cell + gold set only |
...
Five boxes must be dashed: `aut-strong`, `clauseguard check`, `harness/web`,
`harness/extract/extractor.py`, and `harness/probe_gen/`. Two more absences are
worth a footnote strip along the bottom rather than boxes of their own: the gold
set (`tests/gold/gold_labels.jsonl`, empty — so no κ) and `harness/metrics/`
...
flowchart LR
  ...
  ASTRONG["aut-strong<br/>NOT BUILT"]
  ...
  DB -.-> GATE["clauseguard check<br/>THE GATE - NOT BUILT"]
  ...
  class EXTRACT,GEN,ASTRONG,GATE,WEB stub
...
and the gate as dashed ghosts. The barrier and the answer-key chip are the two
